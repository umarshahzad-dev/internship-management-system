...
import { useCompanies, useInternships } from '../internships/internship.queries'
...
export function DashboardPage({ role }: { role: UserRole }) {
  ...
  const internships = useInternships(role === 'ADMIN' ? departmentId : undefined)
  ...
  const rows = (internships.data ?? []).map((item) => ({ ...item, companyName: item.companyName ?? companies.data?.find((company) => company.id === item.companyId)?.name }))
  const cards = role === 'STUDENT'
    ...
    : role === 'ADMIN'
      ? [{ label: 'Toplam başvuru', value: rows.length, icon: BriefcaseBusiness }, { label: 'Onay bekleyen', value: countByStatus(rows, ['PENDING_EMPLOYER', 'PENDING_COMMISSION', 'SUBMITTED']), icon: Clock3 }, { label: 'Tamamlanan', value: countByStatus(rows, ['COMPLETED']), icon: CheckCircle2 }]
      : [{ label: 'Bölüm başvuruları', value: rows.length, icon: BriefcaseBusiness }, { label: 'İşlem bekleyen', value: countByStatus(rows, ['PENDING_EMPLOYER', 'PENDING_COMMISSION', 'APPROVED_PENDING_SGK', 'EVALUATION', 'SUBMITTED']), icon: Clock3 }, { label: 'Belge kontrolü', value: countByStatus(rows, ['APPROVED', 'APPROVED_PENDING_SGK', 'ONGOING']), icon: FileCheck2 }]
